...
import { getProvider } from "./matches";




export async function enqueueTask(ctx: ActionCtx, type: TaskType, triggeredBy: "user" | "task" | "cron" | "system", args: any, previousTaskId?: Id<"tasks">, cronId?: string) {
...
export async function enqueueTaskRerun(ctx: ActionCtx, taskId: Id<"tasks">) {
  ...
  await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "queued", queuedAt: Date.now(), errorMessage: "", progress: 0, progressMessages: [], runAt: undefined, stoppedAt: undefined, attempts: task.attempts + 1, metadata: {}, triggeredBy: "user", triggeredById: user._id, });
...
export const updateTask = internalMutation({
  args: {
    ...
    progressMessages: v.optional(v.array(v.object({
      message: v.string(),
      timestamp: v.number(),
    }))),
  ...
  handler: async (ctx, args) => {
    const { taskId, progressMessage, stoppedAt, ...optionalFields } = args;
    ...
    if (args.progressMessages) { //If progressMessages is provided, overwrite the progressMessages
...
export const task_tt_sync = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    //set task to running
    await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "running", runAt: Date.now(), progress: 0, progressMessage: "Starting sync" });
    ...
    try {
      ...
      await ctx.runMutation(internal.tasks.updateTask, { taskId, progressMessage: "Syncing " + candidates.length + " candidates from Teamtailor" });
      let progress = 0
      ...
      for (let i = 0; i < candidates.length; i++) {
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress });
      ...
      await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "failed", stoppedAt: Date.now(), progressMessage: "Syncing candidates failed", errorMessage: error instanceof Error ? error.message : "Unknown error occurred when syncing candidates" });
      return { success: false, message: "Sync failed" };
    }

    try {
      const jobs = await ctx.runAction(internal.teamtailor.getJobsByUpdatedTT, { updatedAtTT: updatedAt || Date.now() - timeAgo! });
      await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Syncing " + jobs.length + " jobs from Teamtailor" });
      let progress = 50;
      for (let i = 0; i < jobs.length; i++) {
        const job = jobs[i];
        await ctx.runMutation(internal.teamtailor.upsertJobTTCacheRow, { teamtailorId: job.id, updatedAt: Date.parse(job.attributes["updated-at"]), createdAt: Date.parse(job.attributes["created-at"]), title: job.attributes.title, body: job.attributes.body, status: job.attributes.status, internalName: job.attributes["internal-name"] ?? "" });

        progress = 50 + Math.round(i * (50 / jobs.length));
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress });
      }
    } catch (error) {
      await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "failed", stoppedAt: Date.now(), progressMessage: "Syncing jobs failed", errorMessage: error instanceof Error ? error.message : "Unknown error occurred when syncing jobs" });
      return { success: false, message: "Sync failed" };
    }
...
export const task_tt_import = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    //set task to running
    await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "running", runAt: Date.now(), progressMessage: "Starting import" });


    if (type === "candidate") {
      try {
        //Step 1: import candidate
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 10, progressMessage: "Fetching candidate from Teamtailor" });

        const candidate = await ctx.runAction(internal.teamtailor.importCandidate, { teamtailorId });

        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 30, progressMessage: "Creating candidate record in database" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Fetching assesment from Teamtailor" });
        const assesment = await ctx.runAction(internal.teamtailor.fetchCandidateAssesment, { teamtailorId });

        //Step 4: fetch hubert answers
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 70, progressMessage: "Fetching Hubert" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 80, progressMessage: "Creating candidate source data" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, stoppedAt: Date.now(), progress: 90, progressMessage: "Queueing build profile task" });
        await enqueueTask(ctx, "build_profile", "task", { type: "candidate", id: candidateId }, taskId);
        await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "succeeded", stoppedAt: Date.now(), progress: 100, progressMessage: "Import complete" });
    ...
    if (type === "job") {
      try {
        //Step 1: import job
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 10, progressMessage: "Fetching job from Teamtailor" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 30, progressMessage: "Connecting or creating company record" });
        const companyId = await ctx.runMutation(internal.companies.connectOrCreate, { name: job.companyName ?? "Unknown" });

        //Step 2: create job record
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Creating job record in database" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 75, progressMessage: "Creating job source data" });
        await ctx.runMutation(internal.jobs.upsertSourceData, {
          jobId,
          teamtailorBody: job.body,
        });
        //Step 3: enqueue build profile task
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 90, progressMessage: "Queueing build profile task" });
...
export const task_build_profile = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const MODEL = "gpt-5";
    //set task to running
    await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "running", runAt: Date.now(), progressMessage: "Starting build profile" });
    ...
    if (type === "candidate") {
      try {
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 25, progressMessage: "Querying candidate source data" });

        const candidateSourceData = await ctx.runQuery(api.candidates.getSourceData, { candidateId });
        //Step 2: build candidate profile
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Requesting profile from " + getProvider(MODEL) });

        const { profile, metadata } = await ctx.runAction(internal.profiles.buildCandidateProfile, { assessment: candidateSourceData?.assessment, hubertAnswers: candidateSourceData?.hubertAnswers, resumeSummary: candidateSourceData?.resumeSummary, linkedinSummary: candidateSourceData?.linkedinSummary, model: MODEL });


        //Step 3: upsert candidate profile
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 75, progressMessage: "Creating candidate profile in database", metadata: metadata });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 90, progressMessage: "Queueing embed profile task" });
        await enqueueTask(ctx, "embed_profile", "task", { type: "candidate", id: candidateId }, taskId);
        await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "succeeded", progress: 100, progressMessage: "Build profile complete" });
    ...
    if (type === "job") {
      try {
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 25, progressMessage: "Querying job source data" });

        const jobSourceData = await ctx.runQuery(api.jobs.getSourceData, { jobId });

        //Step 2: build job profile
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Requesting profile from " + getProvider(MODEL) });
        const { profile, metadata } = await ctx.runAction(internal.profiles.buildJobProfile, { teamTailorBody: jobSourceData?.teamtailorBody, body: jobSourceData?.body, model: MODEL });

        //Step 3: upsert job profile
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 75, progressMessage: "Creating job profile in database" });
...
export const task_embed_profile = internalAction({
  ...
  handler: async (ctx, args) => {
    const { taskId, type, id } = args;
    //set task to running
    await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "running", runAt: Date.now(), progressMessage: "Starting embed profile" });
    ...
    if (type === "candidate") {
      try {
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 25, progressMessage: "Querying candidate profile" });
        if (!candidateProfile) throw new Error("Candidate profile not found");

        //Step2: check if all fields are present
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Validating candidate profile" });
        validateCandidateProfile(candidateProfile);

        //Step 3: embed candidate profile
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 75, progressMessage: "Embedding candidate profile" });
        ...
        const { embeddings, metadata } = await ctx.runAction(internal.embeddings.embedMany, { texts: textsToEmbed });

        //Step 4: Create candidate embeddings
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 90, progressMessage: "Creating candidate embeddings in database" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "succeeded", progress: 100, stoppedAt: Date.now(), progressMessage: "Embedding done" });
    ...
    if (type === "job") {
      try {
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 25, progressMessage: "Querying job profile" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Validating job profile" });

        validateJobProfile(jobProfile);

        //Step 3: embed job profile
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 75, progressMessage: "Embedding job profile" });
        ...
        await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 90, progressMessage: "Creating job embeddings in database" });
...
export const task_match = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    //set task to running
    await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "running", runAt: Date.now(), progressMessage: "Starting match" });
    ...
    try {
      ...
      await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 25, progressMessage: "Querying profiles" });
      const candidateProfile = await ctx.runQuery(api.candidates.getProfile, { candidateId });
      if (!candidateProfile) throw new Error("Candidate profile not found");
      const jobProfile = await ctx.runQuery(api.jobs.getProfile, { jobId });
      if (!jobProfile) throw new Error("Job profile not found");
      ...
      await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 35, progressMessage: "Querying scoring guidelines" });
      ...
      //Step 3: match
      await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 50, progressMessage: "Requesting match from " + getProvider(model) });
      ...
      //Step 4: create match
      await ctx.runMutation(internal.tasks.updateTask, { taskId, progress: 75, progressMessage: "Creating match record in database", metadata: metadata });
      ...
      await ctx.runMutation(internal.tasks.updateTask, { taskId, status: "succeeded", progress: 100, stoppedAt: Date.now(), progressMessage: "Match done" });
...
export const runTask = action({
  ...
  returns: v.object({ success: v.boolean(), message: v.string(), data: v.object({ taskId: v.optional(v.id("tasks")) }) }),
  handler: async (ctx, args): Promise<{ success: boolean; message: string; data: { taskId?: Id<"tasks"> } }> => {
    try {
      if (args.triggeredBy === "cron") {
        const { taskId } = await enqueueTask(ctx, args.taskType, args.triggeredBy, args.args, undefined, args.triggeredById as string);
        return { success: true, message: "Task queued", data: { taskId } };
      } else {
        const { taskId } = await enqueueTask(ctx, args.taskType, args.triggeredBy, args.args);
        return { success: true, message: "Task queued", data: { taskId } };
      }
    } catch (error) {
      console.error(error);
      return { success: false, message: "Task could not be queued", data: { taskId: undefined } };
    }
  },
});
